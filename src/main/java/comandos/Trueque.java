...
/**
 * Clase que administra el comando de trueque. <br>
 */
public class Trueque extends ComandosEscucha {
    /**
     * Ejecuta el truque entre personajes. <br>
     */
    @Override
    public final void ejecutar() {
        ...
        int inteligencia = juego.getCliente().getPaquetePersonaje()
                .getInteligencia();
        int experiencia = juego.getCliente().getPaquetePersonaje()
                .getExperiencia();
        ...
        try {
            casta = (Casta) Class.forName("dominio" + "."
                    + juego.getCliente().getPaquetePersonaje().getCasta())
                    .newInstance();
            pj = (Personaje) Class
                    .forName("dominio" + "."
        ...
        if (id == paqueteComerciar.getId()) {
            ...
            ArrayList<Item> items = juego.getPersonajesConectados()
                    .get(paqueteComerciar.getIdEnemigo()).getItems();
            boolean loop = true;
            int i = 0;
            while (juego.getCliente().getM1().getObtener().size() > 0) {
                while (loop) {
                    if (items.get(i).getNombre().equals(
                            juego.getCliente().getM1().getObtener().get(0))) {
            ...
            pj.trueque(juego.getCliente().getPaquetePersonaje().getItems(),
                    paqueteComerciar.getItemsADar(),
