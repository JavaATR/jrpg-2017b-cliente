...
public class Entidad {
					...
					if (juego.getEstadoJuego().getMenuEnemigo()
						...
						if (juego.getEstadoJuego().getTipoSolicitud() == MenuInfoPersonaje.MENU_BATALLAR) {
							...
							if (!((int) comercio[0] >= NUM_44
									...
									&& (int) comercio[1] >= 0 && (int) comercio[1] <= NUM_29)) {
	...
	public final void graficarNombre(final Graphics g) {
		...
		Pantalla.centerString(g, new java.awt.Rectangle(drawX + NUM_32, drawY
				- NUM_20, 0, NUM_10), nombre);
	}
	...
	private BufferedImage getFrameAnimacionActual() {
		...
		return tipoMovimiento.get(movimientoHacia) != null ? tipoMovimiento
				.get(movimientoHacia) : Recursos.orco.get(SEIS)[0];
	}
	...
	private int getFrame() {
		...
		tipoMovimiento.put(DIAGONAL_INFERIOR_DERECHA, moverAbajoDer.getFrame());
		tipoMovimiento.put(DIAGONAL_SUPERIOR_IZQUIERDA,
				moverArribaIzq.getFrame());
		tipoMovimiento
				.put(DIAGONAL_SUPERIOR_DERECHA, moverArribaDer.getFrame());

		return tipoMovimiento.get(movimientoHacia) != null ? tipoMovimiento
				.get(movimientoHacia) : 0;
	...
	private void enviarPosicion() {
		...
		try {
			juego.getCliente()
					.getSalida()
					.writeObject(
							gson.toJson(juego.getUbicacionPersonaje(),
	...
	private PilaDeTiles caminoMasCorto(final int xInicial, final int yInicial,
		...
		while (cantSolucion < grafoLibres.obtenerCantidadDeNodosTotal()) {
			...
			for (int i = 0; i < grafoLibres.obtenerNodos()[indiceMinimo]
				...
				if (vecCostos[indiceMinimo] + valorASumar < vecCostos[adyacentes[i]
						.obtenerIndice()]) {
		...
		while (nodoFinal != nodoInicial) {
			camino.push(new NodoDePila(grafoLibres.obtenerNodos()[nodoFinal]
					.obtenerX(), grafoLibres.obtenerNodos()[nodoFinal]
					.obtenerY()));
	...
	private boolean estanEnDiagonal(final Nodo nodoUno, final Nodo nodoDos) {
		return (nodoUno.obtenerX() == nodoDos.obtenerX() || nodoUno.obtenerY() == nodoDos
				.obtenerY());
	}
